...
/*
if we are declearing any type before fuunction,
cpp cmpiler assumes that the return type is int.
...
/*
if you think the function should not return anything at all,
...
/*
The purpose of inline function is to modularize the code
and also to save memory.

which means if func() is a ordinary function and it is being called
the specific memory address is assesed.

in cpp inline function serves 2 purposes,

1. It serves as a compiler directive that suggests (but does not require) that the
compiler substitute the body of the function inline by performing inline expansion,
i.e. by inserting the function code at the address of each function call, thereby
saving the overhead of a function call. In this respect it is analogous to the
register storage class specifier, which similarly provides an optimization hint.

2. The second purpose of inline is to change linkage behavior; the details of this
are complicated. This is necessary due to the C/C++ separate compilation + linkage model,
specifically because the definition (body) of the function must be duplicated in
all translation units where it is used, to allow inlining during compiling, which,
if the function has external linkage, causes a collision during linking (it violates
uniqueness of external symbols). C and C++ (and dialects such as GNU C and Visual
C++) resolve this in different ways.
*/
...
/*
a function do some thing with integer, if i want to do the same thing with
double type or long type, we do function overloading.
...
int main()
{
    ...
    */
    ...
    mean while, if the function is an inline funciton the statements of the function
    will be copied to the point where it is called. apparently we can not see it,
    but compiler can.
    */
